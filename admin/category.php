...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM product WHERE category_id = ?");
    $stmt->execute([$category_id]);
    $productCount = $stmt->fetchColumn();
    if ($productCount > 0) {

        // ถำ้มสี นิ คำ้อยใู่ นหมวดหมนู่ ี้
        $_SESSION['error'] = "ไม่สำมำรถลบหมวดหมนู่ ไี้ ด ้เนอื่ งจำกยังมสี นิ คำ้ทใี่ ชง้ำนหมวดหมนู่ อี้ ย";
    } else {
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" class="d-flex">
                            ...
                            <input type="text" name="new_name" class="form-control me-2" placeholder="ชอื่ ใหม" ่ required>
